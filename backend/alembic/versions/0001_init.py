...
def upgrade() -> None:
    ...
    # Roles
    conn.execute(sa.text(""        INSERT INTO role (name, description)
        VALUES (:name, :desc)
        ON CONFLICT (name) DO NOTHING
    """), [{"name":"admin","desc":"Администратор"},
             {"name":"user","desc":"Пользователь"}])

    # Permissions (basic sample)
    perm_names = [
    ...
    conn.execute(sa.text(""        INSERT INTO permission (name, description)
        VALUES (:name, :desc)
        ON CONFLICT (name) DO NOTHING
    """), [{"name":n, "desc":d} for n,d in perm_names])

    # Grant admin permissions
    conn.execute(sa.text(""        INSERT INTO role_permissions (role_id, permission_id)
        SELECT r.id, p.id
    ...
    for idx,(title,url,icon) in enumerate(links):
        conn.execute(sa.text(""            INSERT INTO admin_menu_link (title, url, icon, "order")
            VALUES (:title, :url, :icon, :ord)
            ON CONFLICT DO NOTHING
        """), dict(title=title, url=url, icon=icon, ord=idx))
